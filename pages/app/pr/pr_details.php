...
//get detail
$sql_farms = "SELECT * FROM emun_pr WHERE id = '$er_Id'";
$query_farms = mysqli_query($conn, $sql_farms);
$u_check_farms = mysqli_num_rows($query_farms);
//get records
$sql_records = "SELECT * FROM pr_records WHERE cp_id = '$er_Id'";
...
if($u_check_farms > 0){
    ...
    $emergency = $row['complication'];
    $er_image = $row['cp_image'];
    $p_id = $row['user_id'];
    $ed_code = $row['ed_code'];
    $contact_of_payment = $row['contact'];
    $date_created = $row['appointment_date'];
    $assign_date = $row['date_created'];
    $status = $row['status'];
    if($status == 2){
...
if(isset($_POST['assign'])){
    ...
    //update emergency
    mysqli_query($conn, "update emun_pr set ed_code ='$emun_doctor', status = 1 where id='$er_Id' ")or die(mysqli_query($conn));

}
//mark emergency as complete
if(isset($_POST['complete'])){
    //update emergency
    mysqli_query($conn, "update emun_pr set status = 2 where id='$er_Id' ")or die(mysqli_query($conn));
}
//upload records
if(isset($_POST['send'])){
    ...
    $sql_UID = "SELECT * FROM pr_records order  by id DESC Limit 1";
    $query_UID = mysqli_query($conn, $sql_UID);
    $u_check_UID = mysqli_num_rows($query_UID);
    $ER = "RECPR".rand(1000,2000);
    if($u_check_UID > 0){
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["er_image"]["tmp_name"], $target_file)) {
            ...
            $messageInsertSQL = "INSERT INTO pr_records (user_id, record_upload, ed_code, cp_id)
...
if($_SERVER['REQUEST_METHOD']=='POST'){
    echo ("<SCRIPT LANGUAGE='JavaScript'>
         window.location.href='$BASEURL?page=pr_details&eId=$er_Id&message=$error'
     </SCRIPT>");
}
?>
        ...
        <div class="card">
            <div class="card-body">
                <div class="d-sm-flex justify-content-between align-items-start my_card_button">
                    <div>
                        <h4 class="card-title card-title-dash">eMUN Appointment</h4>
                    </div>
                    <div>
                ...
                <div class="row">
                    <div class="col-md-6 grid-margin stretch-card">
                        <div class="row">
                            <div class="col-md-6 grid-margin stretch-card">
                                <img src="<?php echo $er_image; ?>" class="my_logo"/>
                            </div>
                            <div class="col-md-6 grid-margin stretch-card">
                                <span class="text-secondary"><h6 class="card-title">Appointment Details</h6><br/>
                                Emergency: <?php echo $emergency; ?><br/>
                                eMun Doctor: <?php echo $ed_code; ?><br/>
                                Date & Time: <?php echo $date_created; ?><br/>
                                Contact of payment: <?php echo $contact_of_payment; ?><br/>
                                Status: <?php echo $statusMessage;
                                if($status == 1) { ?><br/>
                                 <?php if($account_type == 'user'){ ?>
                                     Appointment Payment Confirmed<br/>
                    ...
                    <div class="col-md-6 grid-margin stretch-card">
                        <div class="table-responsive  mt-1">
                            <table class="table select-table">
                                ...
                                <tr>
                                    <th colspan="4">
                                        <h6 class="card-title">Appointment Records: <?php echo $error; ?></h6>
                                    </th>
                                </tr>
                                <tr>
                                    <th>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form class="forms-sample" action="<?php echo $BASEURL; ?>?page=pr_details" method="post" enctype="multipart/form-data">
